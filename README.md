...
除模型自带的通用网页搜索外，系统会按 BP 领域自动调用一组无需 API Key、无需付费账户的结构化研究工具：ClinicalTrials.gov 临床试验、Crossref 论文 DOI、GitHub 公开仓库、Hugging Face 公开模型、OSV/NVD 漏洞、SEC EDGAR 上市公司披露，以及 TED 欧盟采购公告。工具调用有查询数、结果数、超时和重试预算；单一接口限流或不可用时会显示降级提示，并继续保留其他接口已经取得的证据。`GET /api/health` 的 `zeroKeyResearchTools` 会列出当前启用的工具。
