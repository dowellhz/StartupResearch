...
export function resolveReportCitations(markdown, { sources = [], outputLanguage } = {}) {
  ...
  const unverified = createUnverifiedReferenceIndex(markdown, sources);
  ...
  const link = (id) => {
    const extra = unverified.byId.get(id);
    if (extra) unresolved.add(id);
    references.set(id, extra || { ...sourceById.get(id), url: urls.get(id) });
    return `[${english ? "Source" : "来源"} ${id.slice(7)}](#${id})`;
  };
  const report = String(markdown || "").split("\n").map((line) => {
    ...
    // Keep code, images and bare URLs intact. Convert complete citation tokens
    // as a unit so replacing their label cannot create nested links.
    return line.replace(/<a id="source_\d+"><\/a>|!?\[[^\]\n]*\]\((?:\\.|[^)\\\n])*\)|!?\[[^\]\n]*\]\[[^\]\n]*\]|`+[^`]*`+|https?:\/\/[^\s<>]+|\[[^\]\n]+\]:\s*\S+|(?<![\w/])(?:\[(source_\d+)\]|(source_\d+))(?!\w)/g,
      (token, bracketed, bare, offset) => {
        ...
        if (existing) {
          ...
          if (fragmentId && (urls.get(fragmentId) || unverified.byId.has(fragmentId))) return link(fragmentId);
          const namedId = existing[1].match(/^(?:来源 |Source |source_)(\d+)$/)?.[1];
          const url = citationUrl(existing[2]);
          const exact = [...urls].filter(([, value]) => value && value === url);
          const matches = (exact.length ? exact : [...urls].filter(([, value]) => value && citationDocumentUrl(value) === citationDocumentUrl(url)))
            .sort(([left], [right]) => Number(left.slice(7)) - Number(right.slice(7)));
          const id = namedId && urls.get(`source_${namedId}`) === url ? `source_${namedId}` : matches.length ? matches[0][0] : "";
          if (id) return link(id);
          if (url) return link(unverified.get(url, existing[1]).id);
        }
