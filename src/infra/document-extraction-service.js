...
export function createDocumentExtractionService({ maxBytes = 20 * 1024 * 1024, pdfExtractor } = {}) {
  async function extract({ buffer, filename = "business-plan.pdf", mimeType = "" } = {}, context = {}) {
    ...
    try {
      ...
      if (ext === ".pdf" || mimeType === "application/pdf") {
        if (!pdfExtractor?.extract) throw new Error("PDF 解析服务未配置");
        extracted = await pdfExtractor.extract({ buffer }, context);
      }
