...
test("reanalyze controller uses task-specific copy and updates the running snapshot", async () => {
  ...
  const calls = [];
  const run = createReanalyzeController({ state, confirmImpl: () => true, requestJson: async () => ({ review: { id: "industry_1", taskType: "industry_research", stages: [{ key: "plan" }] } }), renderProgress: () => calls.push("render"), connectEvents: (id) => calls.push(id), notify: (message) => calls.push(message), labelFor: () => ({ rerun: "重新研究" }) });
  await run();
  assert.match(confirmationMessage("industry_research"), /重新规划/);
  assert.equal(state.report, "");
  assert.deepEqual(calls.slice(0, 2), ["render", "industry_1"]);
});
