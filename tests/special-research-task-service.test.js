...
test("special research task service persists industry, technology and paper jobs with explicit pipelines", async () => {
  const jobs = [];
  const uploads = [];
  const queued = [];
  const repository = {
  ...
  const technology = await service.create({ taskType: "technology_research", companyName: "fNIRS + EEG + TI", outputLanguage: "zh", researchTemplate: "investment" }, { ownerId: "browser-1" });
  ...
  assert.equal(technology.taskType, "technology_research");
  assert.equal(technology.researchTemplate, "technical");
  assert.equal(industry.outputLanguage, "en");
  assert.equal(paper.outputLanguage, "en");
  assert.equal(paper.upload.persisted, true);
  assert.equal(uploads.length, 1);
  assert.deepEqual(queued, [industry.id, technology.id, paper.id]);
});
