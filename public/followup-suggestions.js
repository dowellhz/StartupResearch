...
const ENGLISH_SUGGESTIONS = [
  "Core capability｜Which capabilities or technologies create real differentiation?",
  "Technical validation｜Which metrics, benchmarks and third-party evidence are still needed?",
  "Industry research｜What drives competition and growth over the next three years?",
  "Due diligence｜Which materials should be requested in the next diligence round?"
];
...
export function renderFollowupSuggestions(container, suggestions, onSelect) {
  ...
  container.innerHTML = `
    <strong>${getLanguage() === LANGUAGE_EN ? "You can continue asking" : "你可以继续问"}</strong>
...
function suggestionButton(question) {
  ...
  return `<button type="button" data-followup-suggestion="${escapeHtml(question)}" aria-label="${getLanguage() === LANGUAGE_EN ? "Ask: " : "发起追问："}${escapeHtml(question)}">${hasCategory ? `<span>${escapeHtml(category)}</span>` : ""}${escapeHtml(visibleQuestion)}</button>`;
}
