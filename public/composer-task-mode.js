...
export function createComposerTaskModeController({ elements, state, clearAttachment = () => {} }) {
  ...
  function selectIndustryResearchMode() {
    ...
    hideSpecialPreviews();
    setNewTaskTitle("新建行业研究");
    elements.industryResearchPreview.classList.remove("hidden");
    elements.companyInput.disabled = false;
    elements.companyInput.placeholder = "行业或技术主题（必填）";
    elements.promptInput.placeholder = "补充研究范围、地区、时间或投资关注点（可选）";
    elements.composerNote.textContent = "行业研究模式：系统会规划问题并检索公开网页、论文与专项数据库";
  }

  function selectPaperAnalysisMode() {
    state.taskType = PAPER_ANALYSIS;
    hideSpecialPreviews();
    setNewTaskTitle("新建论文解读");
    elements.paperAnalysisPreview.classList.remove("hidden");
    elements.fileInput.accept = ".pdf,application/pdf";
    elements.companyInput.disabled = false;
    elements.companyInput.placeholder = "论文标题（可选，可从 PDF 识别）";
    elements.promptInput.placeholder = "补充关注角度（技术、复现、产业化等）";
    elements.composerNote.textContent = "论文解读模式：上传 PDF 或填写公开论文 URL，系统将补充学术检索";
  }
