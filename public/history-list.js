  ...
  container.innerHTML = reviews.map((review) => `
    <div class="history-row ${review.id === currentId ? "active" : ""}">
      <button class="history-item" data-review-id="${escapeHtml(review.id)}">
        ...
        <span><i class="history-status ${escapeHtml(review.status)}"></i>${statusLabel(review.status, review.taskType)} · ${relativeDate(review.createdAt || review.updatedAt)}</span>
      </button>
...
function statusLabel(status, taskType) {
  ...
  return ({ queued: t("status.queued", { zh: "排队中" }), completed: t("status.completed", { zh: "已完成" }), needs_attention: t("status.attention", { zh: "需关注" }), failed: t("status.failed", { zh: "失败" }), cancelled: t("status.cancelled", { zh: "已停止" }) })[status] || status;
}
