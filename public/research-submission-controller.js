...
export function validateInput({ taskType, subject, sourceUrl, file }) {
  if (taskType === COMPANY_PRE_RESEARCH && !subject) return "公司预研需要填写公司名称";
  if (taskType === INDUSTRY_RESEARCH && !subject) return "行业研究需要填写行业或技术主题";
  if (taskType === PAPER_ANALYSIS && !file && !/^https?:\/\//i.test(sourceUrl)) return "论文解读需要上传 PDF 或填写论文 URL";
  if (taskType === PAPER_ANALYSIS && file && !/\.pdf$/i.test(file.name || "") && !/application\/pdf/i.test(file.type || "")) return "论文解读仅支持 PDF 文件";
  return "";
}

function defaultInstruction(taskType) {
  return ({
    company_pre_research: "基于公开信息完成公司预研",
    industry_research: "完成行业概览研究",
    paper_analysis: "从技术、可信度、行业价值和商业化角度解读论文"
  })[taskType] || "完成研究";
}
